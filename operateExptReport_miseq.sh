...
Rscript -e "library(rmarkdown); render(\"${SCRIPTDIR}/expt_report.Rmd\", output_format = \"pdf_document\", output_dir=\"$m_dir\", intermediates_dir=\"$m_dir\", output_file = \"$m_out\", params = list(input=\"$m_file\"))"
